...
def full_process(data):
    ...
    lock.release()
    print("done %d!" % name_number)
    index = name_number - 1
    #result[index] = array_to_write
    lock.acquire()
    name_number += 1
    lock.release()
...
for i in x:
    print("i", i)
    threads[i] = threading.Thread(target=full_process, args=(list_of_bot_data[i], results,))
    threads[i].start()
...
#
# i = 1
# for lists in to_write_list:
#     file_name = "bot_%d_path" % i
#     write_map_array(lists, file_name)
#     i += 1
# t1.join()
# t2.join()
# t3.join()
